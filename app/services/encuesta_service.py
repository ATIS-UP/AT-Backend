"""service layer for survey admin management (CRUD, publishing, results)."""
import uuid
...
class EncuestaService:
    ...
    def listar(self, pagina: int, por_pagina: int, estado: Optional[str] = None) -> tuple[list, int]:
        ...
        total = query.count()
        encuestas = query.order_by(Encuesta.created_at.desc()).offset((pagina - 1) * por_pagina).limit(por_pagina).all()
        return [self._to_dict(e) for e in encuestas], total

    def obtener(self, encuesta_id: str) -> dict:
        return self._to_dict(self._get_or_raise(encuesta_id))
    ...
    def actualizar(self, encuesta_id: str, data: dict, usuario_id: str) -> dict:
        ...
        if encuesta.estado not in ("BORRADOR", "PUBLICADA"):
            raise ValidationError("Solo se pueden editar encuestas en estado BORRADOR o PUBLICADA")
    ...
    def procesar_vencimientos(self) -> dict:
        ...
        candidatas = (
            ...
            .filter(Encuesta.estado == "PUBLICADA", Encuesta.fecha_fin.isnot(None), Encuesta.fecha_fin <= ahora)
            .all()
        )
        procesadas = len(candidatas)
        cerradas = 0
        for encuesta in candidatas:
            datos_anteriores = {"estado": encuesta.estado, "fecha_fin": (encuesta.fecha_fin.isoformat() if encuesta.fecha_fin else None)}
            encuesta.estado = "CERRADA"
            cerradas += 1
            AuditService.log_actualizar(
                db=self.db, usuario_id=None, entidad="Encuesta", entidad_id=str(encuesta.id),
                datos_anteriores=datos_anteriores, datos_nuevos={"estado": "CERRADA", "motivo": "vencimiento_fecha_fin"},
            )
        self.db.commit()
        return {"cerradas": cerradas, "procesadas": procesadas, "fecha_referencia": ahora.isoformat()}
    ...
    def duplicar(self, encuesta_id: str, usuario_id: str) -> dict:
        ...
        if original.estado not in ("PUBLICADA", "CERRADA"):
            raise ValidationError("Solo se pueden duplicar encuestas en estado PUBLICADA o CERRADA")
        preguntas_originales = original.preguntas or []
        nuevas_preguntas = [{**p, "id": i + 1} for i, p in enumerate(preguntas_originales)]
        clon = Encuesta(
            id=uuid.uuid4(), titulo=f"{original.titulo} (copia)", descripcion=original.descripcion,
            preguntas=nuevas_preguntas, estado="BORRADOR", periodo=original.periodo,
    ...
    def publicar(self, encuesta_id: str, usuario_id: str) -> dict:
        encuesta = self._get_or_raise(encuesta_id)
        if encuesta.estado != "BORRADOR":
            raise ValidationError("Solo se pueden publicar encuestas en estado BORRADOR")
    ...
    def registrar_respuesta(self, encuesta_id: str, estudiante_id: str, respuestas: list) -> dict:
        encuesta = self._get_or_raise(encuesta_id)
        if encuesta.estado != "PUBLICADA":
            raise ValidationError("Solo se pueden responder encuestas en estado PUBLICADA")
        ...
        existing = (
            ...
            .filter(RespuestaEncuesta.encuesta_id == encuesta.id, RespuestaEncuesta.estudiante_id == estudiante_id)
            .first()
        )
        if existing:
            raise DuplicateEntityError(entity="RespuestaEncuesta", field="estudiante_id", value=estudiante_id)
        respuesta = RespuestaEncuesta(id=uuid.uuid4(), encuesta_id=encuesta.id, estudiante_id=estudiante_id, respuestas=respuestas)
    ...
    def obtener_resultados(self, encuesta_id: str) -> dict:
        encuesta = self._get_or_raise(encuesta_id)
        respuestas = self.db.query(RespuestaEncuesta).filter(RespuestaEncuesta.encuesta_id == encuesta.id).all()
        preguntas = encuesta.preguntas or []
            ...
            if tipo == "opcion_multiple":
                ...
                for a in answers:
                    try:
                        ...
                        continue
                promedio = sum(numeric_vals) / len(numeric_vals) if numeric_vals else None
                distribucion = {}
                for v in numeric_vals:
                    distribucion[str(int(v))] = distribucion.get(str(int(v)), 0) + 1
        ...
        return {"encuesta_id": str(encuesta.id), "titulo": encuesta.titulo, "total_respuestas": len(respuestas),
    ...
    def _validar_preguntas(self, preguntas: list) -> None:
        ...
        for i, pregunta in enumerate(preguntas):
            ...
            if "texto" not in pregunta:
                raise ValidationError(f"La pregunta en posicion {i} debe tener un campo 'texto'")
            tipo = pregunta.get("tipo", "texto_libre")
            if tipo not in tipos_validos:
                raise ValidationError(f"Tipo de pregunta invalido: '{tipo}'. Tipos validos: {', '.join(sorted(tipos_validos))}")
            if tipo == "opcion_multiple":
    ...
    def _to_dict(self, encuesta: Encuesta) -> dict:
        return {
            "id": str(encuesta.id), "titulo": encuesta.titulo, "descripcion": encuesta.descripcion,
            "preguntas": encuesta.preguntas or [], "estado": encuesta.estado, "periodo": encuesta.periodo,
    ...
    def _respuesta_to_dict(self, respuesta: RespuestaEncuesta) -> dict:
        return {
            "id": str(respuesta.id), "encuesta_id": str(respuesta.encuesta_id),
            "estudiante_id": str(respuesta.estudiante_id), "respuestas": respuesta.respuestas,
